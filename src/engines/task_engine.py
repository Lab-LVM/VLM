...
class ClassificationTaskEngine(TaskEngine):
    ...
    def classification_n_shot(self, shot=None, **kwargs):
        shot = self.n_shot if shot is None else shot
        self.feature_engine.sampling(shot)
        self.metric.reset()
        ...
        self.metric.prefix = f'{shot}shot'
        return self._output
    ...
    def classification_linear_prob(self, **kwargs):
        ...
        val_loader = DataLoader(self.val_dataset, cfg.train.batch_size, num_workers=4, pin_memory=True)

        for data in tqdm(val_loader, total=len(val_loader), desc=f'LinearProb'):
            x, y = map(lambda z: z.to(self.device), data)
