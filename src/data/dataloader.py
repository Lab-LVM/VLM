...
def base_dataloader(cfg):
    ...
    dataset_train = create_dataset(dataset.dataset_name, root=dataset.root, split=dataset.train, is_training=True)
    dataset_eval = create_dataset(dataset.dataset_name, root=dataset.root, split=dataset.valid, is_training=False)
    ...
    loader_eval = create_loader_v2(
        dataset_eval,
        input_size=tuple(dataset.eval_size),
        batch_size=cfg.train.batch_size,
        is_training=False,
        use_prefetcher=aug.prefetcher,
...
def get_dataloader(cfg):
    ...
    if 'cifar' in cfg.dataset.dataset_name:
        ...
        if cfg.dataset.augmentation.autoaug:
            if cfg.dataset.augmentation.auto_aug:
                logging.warning(f'Timm\' RandAug is replaced by torchvision\'s AutoAug. '
                                f'Check CIFAR configs of data.augmentation. Now "auto_aug" is {cfg.dataset.augmentation.auto_aug}')
