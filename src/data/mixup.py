...
def mixup_prompt(prompts):
    ...
    size = 77
    mixed_prompt = list()
    for p, rp in zip(prompts, prompts.flip(0)):
        p = p[p != EOS][:-1]
        rp = rp[rp != EOS][1:]
        new_prompt = torch.cat([p, AND, rp, EOS])
        new_prompt = torch.cat([new_prompt, *[EOS] * (size - new_prompt.size(0))])
        mixed_prompt.append(new_prompt)

    return torch.stack(mixed_prompt)
...
class FastCollateMixup(Mixup):
    ...
    def __call__(self, batch, _=None):
        ...
        if self.mode == 'elem' or self.mode == 'half':
            lam = self._mix_elem_collate(output, batch, half=False)
        elif self.mode == 'pair':
